...
def build_field_trip_prompt(grade, location):
    ...
    prompt = (
        ...
        f"- 글의 내용과 가장 잘 어울리는 지점에 이미지를 삽입해야 합니다.\n"
        f"- 이미지를 삽입할 때는 반드시 '[IMAGE: \"상세한 이미지 설명\"]' 형식을 사용해야 합니다.\n"
        f"- \"상세한 이미지 설명\"은 생성될 이미지의 장면 묘사(예 사람이 완강기를 사용하여 3층 건물 벽을 내려오는 모습),'단순하고 명확한 일러스트 스타일'이라는 단어로 끝나야 합니다.\n"
        f"- 결과물은 다른 설명 없이 바로 PDF 내용만 나올 수 있도록, 마크다운 형식으로 작성해주세요.\n"
        f"\n이제 작성을 시작해 주세요."
    )
    return prompt
